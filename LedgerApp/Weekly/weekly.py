...
def weekly_tab_ui(year:int, month_name:str, session_state:dict):
    """Weekly summary tab with districts and villages"""
    st.markdown("### 📆 Weekly (Saturday Summary)")

    # --- User-defined districts ---
    districts_input = st.text_input(f"Enter districts for {month_name} (comma-separated)", key=f"weekly_districts_{month_name}")
    ...
    for district in districts:
        village_input = st.text_input(f"Villages for {district} (comma-separated)", key=f"weekly_villages_{district}_{month_name}")
        ...
        with district_tabs[dt_idx]:
            ...
            for v_idx, village in enumerate(villages):
                with village_tabs[v_idx]:
                    key = f"{year}_{month_name}_weekly_{district}_{village}"
                    ...
                    with c3:
                        st.download_button(
                            ...
                            file_name=f"{year}_{month_name}_weekly_{district}_{village}.xlsx",
                            key=f"d_{key}"
                        )
